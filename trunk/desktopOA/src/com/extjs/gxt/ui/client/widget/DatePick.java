 ...
 * </dl>
 */
public class DatePick extends BoxComponent {
  ...
  protected void onRender(Element target, int index) {
    ...
    grid.addClickHandler(new ClickHandler() {
      ...
      public void onClick(ClickEvent event) {
        ...
        ComponentEvent be = new ComponentEvent(DatePick.this, evt);
        onDayClick(be);
      }
    });
    ...
    for (int row = 0; row < 6; row++) {
      for (int col = 0; col < 7; col++) {
        grid.setHTML(row, col, "<a href=#><span style='height:50px'></span></a>");
      }
    }
  ...
  private void update(DateWrapper date) {
    ...
    if (vd != null && el() != null) {
      ...
      for (; i < days; i++) {
        ...
        fly(textNodes[i]).update("" + intDay+"<br>okok");
        d = d.addDays(1);
